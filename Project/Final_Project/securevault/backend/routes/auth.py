...
def keycloak_callback(): #######################################################################
    ...
    access_token = token_data.get("access_token") # 여기서 토큰 받음
    ...
    try:
        ...
        payload = pyjwt.decode(
            access_token,
            pem_key,
            algorithms=["RS256"], # 공격자는 이곳을 안거침, keycloak 로긴 후에만 실행되는 함수임
                                  # 이건 URL로 요청할때만 실행되고 API 요청할 때 실행 안됨. 공격자는 위조 토큰으로 API를 바로 찌름
    ...
    if next_url:
        return redirect(f"{next_url}?token={access_token}&auth_type=keycloak") # access_token을 발급받아 리다이렉트 되는 곳에 그대로 딸려나감
                                                                               # 토큰 URI 노출-> 다른 사이트로 이동 시 referer헤더에 포함됨,
                                                                               # 브라우저 히스토리에 url째로 토큰이 저장됨
                                                                               # 서버 엑세스 로그에 토큰 평문으로 기록됨         
