# OAuth 인증 우회 기반 내부망 탐지·대응 체계 구축
...
| 수행기간 | 2026.05.07 ~ 2026.05.28 |
| 참여인원 | 7인 팀 프로젝트 (본인 담당 파트 중심 기술) |

실제 기업 환경과 유사한 3-tier 네트워크(DMZ / Internal / Client)를 구축하고,  
...
│  - Apache Web Server   192.168.11.12  웹서버, Reverse Proxy      │
│  - OAuth 서버(Keycloak) 192.168.11.15  JWT, Algorithm Confusion │
│  - Bastion Host        192.168.11.55  Pivot / SMB 내부망 정찰     │ 
└────────────────────────────────────────────────────────────────┘
        ↓
┌──────────────────────────────────────────────────────────────┐
...
│  - MySQL DB             192.168.12.44  DB / Port 3306        │
│  - ELK Stack (SIEM)     192.168.12.129 Elasticsearch, Kibana │
└──────────────────────────────────────────────────────────────┘
        ↓
┌──────────────────────────────────────────────────────────────┐
│  Client 영역 (192.168.13.0/24)                                │
│  - Windows Client       192.168.13.10  사용자 PC, 도메인 조인     │
└──────────────────────────────────────────────────────────────┘
```
...
> 2인이 침투 시나리오를 설계했으며, 나머지 팀원은 서버 구축·IDS 컨설팅 등을 담당했습니다. 담당 파트는 **3단계(SMB 내부망 정찰 및 횡적 이동)**입니다.
...
> **담당 파트:** SMB 프로토콜을 활용한 Windows 공유 서버 탐색, 민감 정보 획득, 내부망 횡적 이동(Lateral Movement) 시나리오 설계 및 실습을 담당했습니다.
...
웹 서버(root) → setup.sh에서 로컬 계정 발견
        ↓
Bastion으로 SSH 접속 (192.168.11.55)
        ↓
...
Bastion은 DMZ와 Internal 영역 모두에 접근 가능한 **중간 경유지**로 활용했습니다.
...
**대응 방안:**
- 스크립트 내 자격 증명은 환경변수 또는 Vault(HashiCorp Vault 등)로 분리
